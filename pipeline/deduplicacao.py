...
r'\b(?:cashback|cash\sback|moedas?\s+shopee)\b',
re.I,
)
...
r'flapremios|prime\sday|black\sfriday|esquenta)\b',
re.I,
)
...
RE_LINHA_CUPOM_LISTA = re.compile(
r'(?:r$\s*\d+|\d+\s*%)\s+off(?:\s+em\s+r$\s*\d+)?\s*:\s*[A-Z0-9][A-Z0-9-]{3,19}',
re.I,
)

Padrão "OFF: CODIGO" no título

RE_TITULO_OFF_COD = re.compile(
r'(?:r$\s*\d+|\d+\s*%)\s+off(?:\s+em\s+r$\s*\d+)?\s*:\s*[A-Z0-9][A-Z0-9-]{3,19}',
re.I,
)
