    ...
    async with config._SEM_ENVIO:
        ...
        if norm is not None and ofertas:
                ...
                if candidatos:
                    ...
                    log_out.info(f"🔎 [OVERLAP_MATCH] post:{msg_id_rel} casou por "
                                 f"ofertas_compartilhadas={sorted(set(ofertas) & set(db_ofertas_de_post(msg_id_rel)))} "
                                 f"| candidato={sorted(ofertas)}")
